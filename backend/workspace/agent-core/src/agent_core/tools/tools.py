...
# from agent_core.tools.google_books import GoogleBooksClient
from agent_core.tools.helper import get_current_tool_stream, tool_helper
from agent_core.tools.ncl_scraper import NCLScraper
...
@tool
@tool_helper
async def search_ncl_library(
    tool_call_id: Annotated[str, InjectedToolCallId],
    search_criteria: list,  # Or define a more user-friendly input like query: str
) -> str:
    """Searches the National Central Library (NCL) catalog using advanced search criteria.

    This tool allows searching the NCL catalog by specifying up to three search fields and terms.

    Args:
        search_criteria: Required. A list of tuples, where each tuple contains a search field code (string)
                         and the corresponding search term (string). Maximum of 3 tuples (criteria) allowed.
                         ...
                         *   Search for books with "Artificial Intelligence" in the title:
                            `[("WTI", "Artificial Intelligence")]`
                         *   Search for books written by "李開復" with "人工智慧" in the title:
                            `[("WAU", "李開復"), ("WTI", "人工智慧")]`
                         *   Search for books with ISBN "9789861372541":
                            `[("ISBN", "9789861372541")]`
                         *   Search using three criteria (e.g., Title, Author, Subject):
                            `[("WTI", "Data Science"), ("WAU", "John Doe"), ("WSU", "Machine Learning")]`

    Returns:
        A string summarizing the search results, including title, author, publisher, year, call number, location, and link for each book found.
        Returns a message like "No books found matching the criteria." if no results are retrieved.
        Returns an error message if the search fails.
    """
    stream = get_current_tool_stream()

    scraper = NCLScraper(headless_mode=True)  # Use headless mode for tool execution
    try:
        # Note: We are now calling the synchronous `search` method of the scraper.
        results = scraper.search(search_criteria)

        if not results:
            response = "No books found matching the criteria."
            stream.send_complete(response)
            return response

        # Format the results into a readable string for the LLM
        response_parts = ["Found the following books at NCL:"]
        for i, book in enumerate(results):
            response_parts.append(
                f"{i + 1}. Title: {book.get('title', 'N/A')}\n"
                f"   Author: {book.get('author', 'N/A')}\n"
                f"   Publisher: {book.get('publisher', 'N/A')}\n"
                f"   Year: {book.get('year', 'N/A')}\n"
                f"   Call No: {book.get('call_number', 'N/A')}\n"
                f"   Location: {book.get('location', 'N/A')}\n"
        ...
        error_message = f"An error occurred while searching NCL: {e}"
        # Log the detailed error server-side
        # logger.error(f"NCL Search Tool Error: {e}", exc_info=True) # Requires logger setup in tools.py
        print(error_message)
